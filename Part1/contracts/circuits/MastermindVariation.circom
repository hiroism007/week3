 ...
 * 8 colors, 0,1,2,3,4,5,6,7
 * 5 holes
**/
template SuperMastermind() {
    // Public inputs
    ...
    // Create a constraint that the solution and guess digits are all less than 8 and greater than -1
    for (j=0; j<5; j++) {
        ...
        greaterThan[j+5] = GreaterThan(4);
        greaterThan[j+5].in[0] <== soln[j];
        greaterThan[j+5].in[1] <== -1;
        greaterThan[j+5].out === 1;
 ...
 component main {public [pubGuessA, pubGuessB, pubGuessC, pubGuessD, pubGuessF,pubNumHit, pubNumBlow, pubSolnHash]} = SuperMastermind();